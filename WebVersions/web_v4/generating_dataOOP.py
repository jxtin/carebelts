...
class HealthParameters:
    ...
    def health_critera(
            self):  # This function is used to check the health of the data
        # Generate value now
...
track_data = HealthParameters("carebelt_v4", 100, 98, 98.6)


def post_document(track_data):
    timestamp = datetime.now(
        tz=gettz("Asia/Kolkata")).strftime("%d/%m/%y %H:%M:%S")
    print(timestamp)
    ...
    if data_entry.health_index > 96:
        ...
        create_document_response = client.post_document(
            db=track_data.name, document=data_entry).get_result()
        ...
        track_data.hr_value = 85
        track_data.spo2_value = 99
        track_data.temperature_value = 98.9
